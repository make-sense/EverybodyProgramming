 ...
 */
	class Firmata
	{
		...
		public Firmata(string deviceName, Int32 baudRate, bool autoStart, int delay)
		{
			_serialDevice = new SerialDevice(deviceName, baudRate);
		...
		/// </summary>
		/// <param name="serialPortName">String specifying the name of the serial port. eg COM4</param>
		public Firmata(string serialPortName) : this(serialPortName, 115200, true, 8000) { }
		...
		public Firmata(string serialPortName, Int32 baudRate) : this(serialPortName, baudRate, true, 8000) { }
...
//		/// and automatically opens the specified serial connection.
//		/// </summary>
//		public Firmata() : this(Firmata.list().ElementAt(list().Length - 1), 115200, true, 8000) { }
		...
		public void Open()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				command[1] = (byte)1;
				_serialDevice.Write(command, 0, 2);
			}
		...
		public string[] list()
		{
			return _serialDevice.GetPortNames();
		}
		...
		public void pinMode(int pin, int mode)
		{
			...
			message[1] = (byte)(pin);
			message[2] = (byte)(mode);
			_serialDevice.Write(message, 0, 3);
			message = null;
		}
		...
		{
			return _serialDevice.BytesToRead;
		}
			...
			while (_serialDevice.IsOpen)
			{
				if (_serialDevice.BytesToRead > 0)
				{
					lock (this)
					{
						...
						int inputData = _serialDevice.ReadByte();
						int command;
